    ...
    for pol in data:
      for bl in data[pol]:
        ...
        if opts.df != None:
            ddf = d[:,1:-1] - .5 * (d[:,:-2] + d[:,2:])
            ddf2 = n.abs(ddf)**2
            ddfum = ddf2[n.where(m==0)]
            sig = n.sqrt(n.median(ddfum.reshape(len(data_times),len(ddfum)/len(data_times)), axis=1))
            #sig = n.sqrt(n.median(ddf2[n.where(m)==0], axis=1))
